...
@property (nonatomic, strong) NSString *key;                ///< key 缓存数据的key
@property (nonatomic, strong) NSData *value;                ///< value 缓存数据的value
@property (nullable, nonatomic, strong) NSString *filename; ///< filename (nil if inline) 缓存文件名(文件缓存时有用)
@property (nonatomic) int size;                             ///< value's size in bytes 数据大小
@property (nonatomic) int modTime;                          ///< modification unix timestamp 数据修改时间(用于更新相同key的缓存)
@property (nonatomic) int accessTime;                       ///< last access unix timestamp 数据访问时间
@property (nullable, nonatomic, strong) NSData *extendedData; ///< extended data (nil if no extended data) 附加数据
@end
